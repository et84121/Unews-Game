...
function kwsk(IsTimeLine) { /*原先圖表與1月~3月的圖表轉場*/

    var dur = 2000;

    if (IsTimeLine == true && canButton == true) {
...
function kwsk2(IsTimeLine) { /*原先圖表與1月~3月的圖表轉場*/

    var dur = 2000;

    if (IsTimeLine == false && canButton == true) {
        //IsTimelineChart = true;
        close_PPchart2();
        onlyTimeLineOut();
        d3.timeout(pathAni1, 1000);
        d3.timeout(function () { IsTimelineChart = true; }, 1)
    }
}
...
function Draw_Second_Graph() { /*用以做出2020 第一季圖表 (第二章圖表)*/
    ...
    d3.csv("static/data/2020_Garbage.csv").then(function (data2) {
        ...
        svg.selectAll('rect')
            ...
            .enter()
            .append("rect")
            .attr("id", function (d, i) { return "New_rect" + (i + 1) })
